		...
		switch OPT_GELDZEIT_Fraktionauswahl do 
		{
			case "AAFvsCSAT" : 
			{
				...
				else 
				{
						...
						private _message = format [MLOC(POINT_NO_DOMINATOR_AAFvCSAT), GVAR(aaf_points), GVAR(csat_points)];
						private _timestamp = [serverTime - OPT_GELDZEIT_startTime] call CBA_fnc_formatElapsedTime;
						[_message] remoteExecCall ["systemChat", 0, true];
			...
			case "NATOvsCSAT" : 
			{
				...
				else 
				{
						...
						private _message = format [MLOC(POINT_NO_DOMINATOR_NATOvCSAT), GVAR(nato_points), GVAR(csat_points)];
						private _timestamp = [serverTime - OPT_GELDZEIT_startTime] call CBA_fnc_formatElapsedTime;
						[_message] remoteExecCall ["systemChat", 0, true];
			...
			case "NATOvsAAF" : 
			{
				...
				else 
				{
						...
						private _message = format [MLOC(POINT_NO_DOMINATOR_NATOvAAF), GVAR(nato_points), GVAR(aaf_points)];
						private _timestamp = [serverTime - OPT_GELDZEIT_startTime] call CBA_fnc_formatElapsedTime;
						[_message] remoteExecCall ["systemChat", 0, true];
